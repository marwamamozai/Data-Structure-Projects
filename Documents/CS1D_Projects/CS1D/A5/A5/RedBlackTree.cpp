...
size_t RedBlackTree::rankOfHelper(Node* node, int key) const {
    if (node == nullptr) {
        return 0;
    }
    if (key == node->key) {
    ...
    if (key < node->key) {
        return rankOfHelper(node->left, key);
    }
    return 1 + size(node->left) + rankOfHelper(node->right, key);
}
...
// helper to find node with rank in subtree
RedBlackTree::Node* RedBlackTree::selectHelper(Node* node, size_t rank) const {
    if (node == nullptr) {
        return nullptr;
    }
...
int RedBlackTree::select(size_t rank) const {
    Node* result = selectHelper(root, rank);
    if (result != nullptr) {
        return result->key;
    } else {
